...
# Activations
from app.utils.models.activations import Glu, Swish
...
# Layers
from app.utils.models.layers import Conv1d, DepthwiseSeparableConv1d, Linear, Transpose
    ...
    def __init__(self, num_layers, in_dim, filters, kernel_size, norm, act):
        ...
        self.layers = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Conv1d(
                        in_dim if layer_id == 0 else filters[layer_id - 1],
                        filters[layer_id],
                        kernel_size,
                        stride=2,
    ...
    def __init__(self, num_layers, filters, kernel_size, norm, act):
        ...
        self.layers = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Conv2d(
                        1 if layer_id == 0 else filters[layer_id - 1],
                        filters[layer_id],
                        kernel_size,
                        stride=2,
                        padding=(kernel_size - 1) // 2,
        ...
        self.conv_layers = nn.Sequential(
            *[
                DepthwiseSeparableConv1d(
                    dim_in if layer_id == 0 else dim_out,
                    dim_out,
                    kernel_size,
                    stride if layer_id == num_layers - 1 else 1,
                    causal,
                )
                for layer_id in range(num_layers)
            ]
