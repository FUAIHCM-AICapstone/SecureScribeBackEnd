...
def generate_meeting_note(meeting_state: MeetingState) -> str:
    ...
    for chunk in chunk_summaries:
        ...
        info_count = len(getattr(chunk, "key_points", [])) + len(getattr(chunk, "facts", [])) + len(getattr(chunk, "problems", [])) + len(getattr(chunk, "solutions", [])) + len(getattr(chunk, "risks", [])) + len(getattr(chunk, "next_steps", []))
