...
def crud_create_meeting_bot(db: Session, meeting_id: int, scheduled_start_time: Optional[datetime], meeting_url: Optional[str], created_by: int, bot_id: Optional[int] = None, status: str = "active") -> MeetingBot:
    bot = MeetingBot(meeting_id=meeting_id, scheduled_start_time=scheduled_start_time, meeting_url=meeting_url, created_by=created_by, status=status)
    db.add(bot)
    db.commit()
    db.refresh(bot)
    return bot
...
def crud_get_meeting_bot(db: Session, bot_id: int, include_relations: bool = True) -> Optional[MeetingBot]:
...
def crud_get_meeting_bot_by_meeting(db: Session, meeting_id: int) -> Optional[MeetingBot]:
    return db.query(MeetingBot).filter(MeetingBot.meeting_id == meeting_id).first()


def crud_get_meeting_bots(db: Session, user_id: int, page: int = 1, limit: int = 20) -> Tuple[List[MeetingBot], int]:
    offset = (page - 1) * limit
...
def crud_update_meeting_bot(db: Session, bot_id: int, **updates) -> Optional[MeetingBot]:
...
def crud_delete_meeting_bot(db: Session, bot_id: int) -> bool:
    bot = db.query(MeetingBot).filter(MeetingBot.id == bot_id).first()
    if not bot:
        return False
...
def crud_create_bot_log(db: Session, bot_id: int, action: str, message: Optional[str] = None) -> MeetingBotLog:
...
def crud_get_bot_logs(db: Session, bot_id: int, page: int = 1, limit: int = 50) -> Tuple[List[MeetingBotLog], int]:
    offset = (page - 1) * limit
...
def crud_get_meeting_projects(db: Session, meeting_id: int) -> List[int]:
    return [p[0] for p in db.query(ProjectMeeting.project_id).filter(ProjectMeeting.meeting_id == meeting_id).all()]


def crud_check_user_project_access(db: Session, user_id: int, project_ids: List[int]) -> bool:
    if not project_ids:
        return False
