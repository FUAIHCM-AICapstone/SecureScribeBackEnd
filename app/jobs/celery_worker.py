...
def publish_task_progress_sync(user_id: str, progress: int, status: str,
                              estimated_time: str = None, task_type: str = "test_notification") -> bool:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if success:
                ...
                print(f"Failed to publish task progress for user {user_id} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(base_delay * (2 ** attempt))  # Exponential backoff

        except Exception as e:
            print(f"Error publishing task progress for user {user_id} (attempt {attempt + 1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                time.sleep(base_delay * (2 ** attempt))  # Exponential backoff
...
def send_test_notification_task(self, user_id: str):
    ...
    try:
        ...
        if publish_task_progress_sync(user_id, 0, "started", "30s", "test_notification"):
        ...
        if publish_task_progress_sync(user_id, 25, "processing", "25s", "test_notification"):
            publish_success_count += 1
        print("processing")
        time.sleep(1)
        ...
        if publish_task_progress_sync(user_id, 50, "database", "15s", "test_notification"):
            publish_success_count += 1
        print("database")
        time.sleep(1)
        ...
        if publish_task_progress_sync(user_id, 75, "creating_notification", "10s", "test_notification"):
            publish_success_count += 1
        ...
        if publish_task_progress_sync(user_id, 90, "sending_fcm", "5s", "test_notification"):
            publish_success_count += 1
        print("sending_fcm")
        time.sleep(1)
        ...
        if publish_task_progress_sync(user_id, 100, "completed", "", "test_notification"):
            publish_success_count += 1
        print(f"completed (Redis publish: {publish_success_count}/{total_publish_attempts})")
        return {
            "status": "ok",
