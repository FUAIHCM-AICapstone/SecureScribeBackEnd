...
from app.schemas.chat import (
    ...
    ChatSessionUpdate,
    ChatSessionWithMessagesApiResponse,
)
...
from app.services.chat_agent import create_meeting_chat_agent
...
def get_meeting_chat_sessions_endpoint(
    ...
    sessions, total = get_chat_sessions_for_meeting(
...
async def send_chat_message_endpoint(
    ...
    agent = create_meeting_chat_agent(
...
def get_chat_conversation_endpoint(
    ...
    conversation = get_chat_session_with_messages(
...
async def chat_websocket_endpoint(
    ...
    token: str = Query(None, description="JWT token (legacy)"),
):
    ...
    try:
        ...
        while True:
            try:
                ...
                # Create user message
                user_message = create_chat_message(
                    db, session_id, uuid.UUID(user_id), message_content, ChatMessageType.user
                )
                ...
                if agent:
                    ...
                    # Create agent message
                    agent_message = create_chat_message(
                        db, session_id, uuid.UUID(user_id), agent_response, ChatMessageType.agent
                    )
