...
from app.services.transcript import transcribe_audio_file, get_transcript_by_meeting
...
def get_meeting_transcript(meeting_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
